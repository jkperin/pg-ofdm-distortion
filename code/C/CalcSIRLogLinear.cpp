/*	1. CalcSIRLogLinear:
		a. Função para calcular componentes de intermodulação que serão utilizados  no cálculo do SIR.
		...
		c. Somente IMPs que caem na banda do sinal são computadas.
		d. Função calcula até componente de 5ª ordem, sendo o mínimo de 3ª ordem. 
		e. O cálculo de algumas IMPs são ignorados para garantir compatibilidade com o resultado calculado numericamente .
*/
...
void calcEva12Var(double * Idet2, double * Idet3, double * Idet4, double * Idet5)
{
    ...
    for(int k1 = 0; k1 < Nc; k1++)
    {
        ...
        {   
            fk = -F[k1] + F[k2];
            /* condições fk != F[k1] && fk != F[k2] são necessárias para 
            poder comparar cálculo numérico. Caso contrátio, os IMPs que 
            caem na mesma frequência que os seus tons geradores serão computados
             este tipo de IMPs não é calculada no modelo numérico */
            if(fk >= F[0] && fk <= F[Nc-1] && fk != F[k1] && fk != F[k2]) 
            {
                w = 2*pi*fk;
                nn = -Fi[k1] + Fi[k2];
                nk[k1] = -1; nk[k2] = 1;
            ...
            for(int k3 = k2 + 1; k3 < Nc; k3++)
            {
                for(int i = 0; i < N3; i++)
                {
                    ...
                    w = 2*pi*fk;
                    if(fk >= F[0] && fk <= F[Nc-1] && fk != F[k1] && fk != F[k2] && fk != F[k3])
                    {
                ...
                for(int k4 = k3 + 1; k4 < Nc; k4++)
                {
                    for(int i = 0; i < N4; i++)
                    {
                        ...
                        w = 2*pi*fk;
                        if(fk >= F[0] && fk <= F[Nc-1] && fk != F[k1] && fk != F[k2] && fk != F[k3] && fk != F[k4])
                        {
                    ...
                    for(int k5 = k4 + 1; k5 < Nc; k5++)
                    {
                        for(int i = 0; i < N5; i++)
                        {
                            ...
                            w = 2*pi*fk;
                            if(fk >= F[0] && fk <= F[Nc-1] && fk != F[k1] && fk != F[k2] && fk != F[k3] && fk != F[k4] && fk != F[k5])
                            {
...
double calcvar(const int *nk, int nn, double w)
{
    double Jn, Jnp1, Jnn1, u, theta, ctheta, ProdJn = 1.0;
    complex <double> SDeltaJ = complex<double>(0,0);
    for(int k = 0; k < Nc; k++)
    {
        /*Para garantir compatibilidade com resultados numéricos, a frequência 
        na qual o pulso cai não pode participar do cálculo*/
        if(Fi[k] == nn)
            continue;
        else
        {
...
//             Jnn1 = J(nk[k]-1, u); 
//             Jnp1 = J(nk[k]+1, u); //_jn
            J(Jn, Jnn1, Jnp1, nk[k], u);

            SDeltaJ += (0.5*mIM[k]*ctheta/Jn)*(endPSI[k]*Jnn1 - epdPSI[k]*Jnp1);
            ProdJn *= Jn;
        }
    }
