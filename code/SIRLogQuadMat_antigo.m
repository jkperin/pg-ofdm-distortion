...
[F2, mIM2, mFM2, ~, ~, dPSI2, OrigInd, G] = calc_ind(F, mIM, mFM, phiIM, phiFM, alfa);
...
for kn = 1:Nc
    ...
    pok = 1:Nc; pok(kn) = [];
    [F2, mIM2, mFM2, phiIM2, phiFM2, dPSI2, OrigInd, G] = calc_ind(F(pok), mIM(pok), mFM(pok), phiIM(pok), phiFM(pok), alfa);
    Fi = F2/deltaf;
    wk = 2*pi*F2;
    nk = zeros(1,Nc2);
...
toc
%%
SIRteo = corta(SIRteo, indF);
SIRap1 = corta(SIRap1, indF);
SIRap2 = corta(SIRap2, indF);
SIReva = corta(10*log10(abs(loglinear.Idet)./abs(loglinear.IMPdet)), indF);
SIRevam = corta(10*log10(abs(loglinearm.Idet)./abs(loglinearm.IMPdet)), indF);
SIReva2 = corta(10*log10(abs(logquad.Idet)./abs(logquad.IMPdet)), indF);
