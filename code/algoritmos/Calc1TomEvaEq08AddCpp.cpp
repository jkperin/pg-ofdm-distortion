/** Calcula corrente Idet usando o modelo do artigo da Eva Peral equações 7 e 8 
 levando em conta os termos adicionais que foram omitidos **/
...
inline void calcEvaEq08Add(complex <double> * Idet, double P0, double mIM, double mFM, double dPSI, double theta, int N2);
...
void mexFunction(int nlhs, mxArray *plhs[], int nrhs, const mxArray *prhs[])
{
  ...
  calcEvaEq08Add(Idet, P0, mIM, mFM, dPSI, theta, N2);
...
inline void calcEvaEq08Add(complex <double> * Idet, double P0, double mIM, double mFM, double dPSI, double theta, int N2)
{
    ...
    for(int n = -N2, nn = 0; n < N2; n++)
    {
        ...
        Idet[nn] = P0*jjn*nedPSI*(_jn(n,u)*umc - jj*(mIM/2*cos(n*theta))*(_jn(n-1,u)*edPSIn - _jn(n+1,u)*edPSIp) 
            + umc*(((mIM/2)*(mIM/4))*_jn(n,u)*cos(2*n*theta)) - ((mIM/4)*(mIM/4))*(_jn(n+2,u)*edPSIp*edPSIp + _jn(n-2,u)*edPSIn*edPSIn));
...
// for n = -N2/2:N2/2-1
//     u = 2*mFM*sin(n*theta);
// eva08_add.Idet(nn) = P0*(1j)^(n)*exp(1j*n*dPSI)*(J(n, u) - 1j*mIM/2*cos(n*theta)*(J(n-1,u)*exp(-1j*dPSI) - J(n+1,u)*exp(1j*dPSI))...
//         +2*(mIM/4)^2*J(n,u)*cos(2*n*theta) - (mIM/4)^2*(J(n+2,u)*exp(1j*2*dPSI) + J(n-2,u)*exp(-1j*2*dPSI)));
